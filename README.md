Yapıştırılan markdown(20260825-074448).md
Dosya
README(1).md
Dosya
github statistics kısmı bu şekilde gözüküyor. gerçekten valid ve çalışan istatistikler koyalım. sadece biri 

README’yi güncel kariyer yönüne göre yeniden sıralayacağım. Şunları netleştirelim:

neyi

README’deki hakkındaki eski bilgileri kastettim 😅 Fazla detaylı sordum.

Şunları güncelleyeceğim:

    3rd-year → 4th-year Computer Engineering Student

    GPA → 3.88/4.00

    Akçansa Automation Internship eklenecek

    Kariyer odağı → Backend / Full-Stack + AI/ML ve Health-Tech

    Güncel projelerin öne çıkarılacak

    Eski veya yanlış proje durumları düzeltilecek

    Çalışmayan istatistik kartları çalışan kartlarla değiştirilecek

Bildiğim güncel bilgilerine göre ben düzenleyeyim; eksik kalan olursa sonra birlikte düzeltiriz.

tamam go. tasarımımı bozma

Tamam, tasarımı bozmadan güncelledim:

    Çiçekli banner ve ayraçlara dokunulmadı.

    3rd-year → 4th-year yapıldı.

    Kariyer odağı ve Akçansa stajı eklendi.

    aura.board ve Academic Calendar Scraper eklendi.

    Proje durumlarındaki çelişkiler düzeltildi.

    C# ve .NET 9 rozetleri eklendi.

    Çalışmayan iki istatistik kartı, güncel olarak belgelenen Stats ve Most Commit Language kartlarıyla değiştirildi. (Kaynak)
